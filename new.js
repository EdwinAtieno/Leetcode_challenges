...
function countPairs(numbers) {
  ...
  for (let i = 0; i < numbers.length - 1; i++) {
    ...
    // Check if there is another number with the same first and last digits
    for (let j = i + 1; j < numbers.length; j++) {
      const otherNumber = numbers[j];

      // Check if the last digit of the current number matches the first digit of the other number
      if (currentNumber % 10 === otherNumber / 10) {
        count++;
        break;
      }
    }
