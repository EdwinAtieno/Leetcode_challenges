...
def time_series_models_comparison():
    ...
    prophet_model.fit(train_set.reset_index().rename(columns={"ds": "ds", "y": "y"}))
    exp_smoothing_model = statsmodels.tsa.holtwinters.ExponentialSmoothing(train_set, seasonal_periods=12, trend="add", seasonal="add").fit()
    arima_forecast = arima_model.predict(n_periods=24)
    prophet_forecast = prophet_model.predict(test_set.reset_index().rename(columns={"ds": "ds"}))
    exp_smoothing_forecast = exp_smoothing_model.predict(start=test_set.index[0], end=test_set.index[-1])
    arima_forecast_error = sklearn.metrics.mean_squared_error(test_set, arima_forecast, squared=False)
    prophet_forecast_error = sklearn.metrics.mean_squared_error(test_set, prophet_forecast["yhat"], squared=False)
    exp_smoothing_forecast_error = sklearn.metrics.mean_squared_error(test_set, exp_smoothing_forecast, squared=False)
    return arima_forecast_error, prophet_forecast_error, exp_smoothing_forecast_error